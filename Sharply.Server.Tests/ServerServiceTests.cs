...
public class ServerServiceTests
{
    ...
    [Test]
    public async Task RemoveUserFromServerAsync_Success()
    {
        if (_service == null || _dbOptions == null)
            throw new Exception("Service or DbOptions was null.");
        ...
        using (var seedContext = new SharplyDbContext(_dbOptions))
        {
            ...
            seedContext.Servers.Add(new Models.Server
            {
                Id = 100,
                OwnerId = 1,
                Name = "ServerForRemoval",
                InviteCode = "ABCDEFGH"
            });

            seedContext.Channels.AddRange(
                new Channel { Id = 10, ServerId = 100, Name = "Channel A" },
                new Channel { Id = 11, ServerId = 100, Name = "Channel B" }
            );

            seedContext.UserServers.Add(new UserServer { UserId = 1, ServerId = 100 });

            seedContext.UserChannels.AddRange(
                new UserChannel { UserId = 1, ChannelId = 10 },
                new UserChannel { UserId = 1, ChannelId = 11 }
            );
        ...
        // Act
        var result = await _service.RemoveUserFromServerAsync(1, 100);
        ...
        var userServerCheck = await verificationContext.UserServers
            .FirstOrDefaultAsync(us => us.UserId == 1 && us.ServerId == 100);
        Assert.That(userServerCheck, Is.Null, "UserServer entry was not removed.");

        var userChannelsCheck = await verificationContext.UserChannels
            .Where(uc => uc.UserId == 1 && (uc.ChannelId == 10 || uc.ChannelId == 11))
            .ToListAsync();
        Assert.That(userChannelsCheck.Count, Is.EqualTo(0), "UserChannel entries were not removed.");
    }

    [Test]
    public async Task RemoveUserFromServerAsync_ReturnsFalse_WhenUserNotInServer()
    {
        if (_service == null || _dbOptions == null)
            throw new Exception("Service or DbOptions was null.");
        ...
        using (var seedContext = new SharplyDbContext(_dbOptions))
        {
            seedContext.Users.Add(new User { Id = 2, Username = "AnotherUser" });
            seedContext.Servers.Add(new Models.Server
            {
                Id = 200,
                OwnerId = 2,
                Name = "ServerNotJoined",
                InviteCode = "HIJKLMNO"
            });

            seedContext.Channels.Add(new Channel { Id = 20, ServerId = 200, Name = "Channel X" });

            await seedContext.SaveChangesAsync();
        }

        // Act
        var result = await _service.RemoveUserFromServerAsync(2, 200);
        ...
        var userServersCount = await verificationContext.UserServers.CountAsync();
        var userChannelsCount = await verificationContext.UserChannels.CountAsync();
        Assert.That(userServersCount, Is.EqualTo(0), "Expected no UserServers to be removed or added.");
        Assert.That(userChannelsCount, Is.EqualTo(0), "Expected no UserChannels to be removed or added.");
    }
}
