...
public class ApiServiceTests
{
    ...
    [Test]
    public async Task JoinServerAsync_SuccessfulResponse_ReturnsServerDto()
    {
        if (_apiService == null)
            throw new Exception("_apiService was null");
        ...
        // Arrange
        var tokenString = "dummy_token";
        var joinRequest = new JoinServerRequest
        {
            InviteCode = "ABCDEFGH",
        };

        var mockApiResponse = new ApiResponse<ServerDto>
        {
            Success = true,
            Data = new ServerDto
            {
        ...
        // Return a 200 OK with a ApiResponse<ServerDto>
        _mockHttpMessageHandler?.Protected()
            .Setup<Task<HttpResponseMessage>>(
                "SendAsync",
                ItExpr.Is<HttpRequestMessage>(req =>
                    req.Method == HttpMethod.Post &&
                    req.RequestUri == new Uri("https://localhost:9999/api/servers/join-server")),
                ItExpr.IsAny<CancellationToken>()
            )
            .ReturnsAsync(new HttpResponseMessage
            {
                StatusCode = HttpStatusCode.OK,
                Content = JsonContent.Create(mockApiResponse)
            });

        // Act
        var result = await _apiService.JoinServerAsync(tokenString, joinRequest);
    ...
    [Test]
    public void JoinServerAsync_FailedResponse_ThrowsException()
    {
        if (_apiService == null)
            throw new Exception("_apiService was null");
        ...
        _mockHttpMessageHandler?.Protected()
            ...
            .ReturnsAsync(new HttpResponseMessage
            {
                ...
                Content = new StringContent("Server not found")
            });

        // Act & Assert
        var ex = Assert.ThrowsAsync<Exception>(async () =>
            await _apiService.JoinServerAsync(tokenString, joinRequest)
        );

        Assert.That(ex, Is.Not.Null);
        Assert.That(ex!.Message, Does.Contain("Server returned NotFound in JoinServerAsync"));
    }

    [Test]
    public async Task LeaveServerAsync_SuccessfulResponse_ReturnsTrue()
    {
        if (_apiService == null)
            throw new Exception("_apiService was null");
        ...
        // Arrange
        var tokenString = "dummy_token";
        var leaveRequest = new LeaveServerRequest
        {
            ServerId = 999
        };

        var mockApiResponse = new ApiResponse<bool>
        {
            Success = true,
            Data = true
        };

        // Successful 200 OK response with the boolean result
        _mockHttpMessageHandler?.Protected()
            .Setup<Task<HttpResponseMessage>>(
                "SendAsync",
                ItExpr.Is<HttpRequestMessage>(req =>
                    req.Method == HttpMethod.Post &&
                    req.RequestUri == new Uri("https://localhost:9999/api/servers/leave-server")),
                ItExpr.IsAny<CancellationToken>()
            )
            .ReturnsAsync(new HttpResponseMessage
            {
        ...
        // Act
        var result = await _apiService.LeaveServerAsync(tokenString, leaveRequest);
    ...
    [Test]
    public void LeaveServerAsync_FailedResponse_ThrowsException()
    {
        if (_apiService == null)
            throw new Exception("_apiService was null");
        ...
        _mockHttpMessageHandler?.Protected()
            ...
            {
                StatusCode = HttpStatusCode.InternalServerError,
                Content = new StringContent("Some server error")
            });

        // Act & Assert
        var ex = Assert.ThrowsAsync<Exception>(async () =>
            await _apiService.LeaveServerAsync(tokenString, leaveRequest)
        );

        Assert.That(ex, Is.Not.Null);
        Assert.That(ex!.Message, Does.Contain("Server returned InternalServerError in LeaveServerAsync"));
    }
}
